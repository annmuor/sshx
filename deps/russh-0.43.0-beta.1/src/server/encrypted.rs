...
impl Session {
    ...
    async fn process_packet<H: Handler + Send>(
        ...
        match enc.state {
            ...
            EncryptedState::WaitingAuthRequest(ref mut auth)
            if buf.first() == Some(&msg::TOTP_INFO_RESPONSE) =>
                {
                    let resp = read_totp_response(
                        rejection_wait_until,
                        handler,
                        &mut enc.write,
                        auth,
...
async fn read_totp_response<H: Handler + Send>(
    until: Instant,
    handler: &mut H,
    write: &mut CryptoVec,
    auth_request: &mut AuthRequest,
